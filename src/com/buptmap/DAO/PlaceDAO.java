...
public class PlaceDAO {
	...
	public double[] getLatAndLon(String unit_id){
		...
		try{
			...
			placeList = getHibernateTemplate().find(sql, unit_id);
	...
	public JSONArray passage(String unit_id, int unit_type_id){
		...
		try{
			...
			passageList = getHibernateTemplate().find(sql, params);
	...
	/**fuzzyQuery extend by yh
	 */
	
	public JSONArray fuzzyQuery(String spotname, String unitId) {
		try {
		...
		spotList = this.hibernateTemplate.find("select show_name,parent_unit_id,parent_name,floor_id,coord_x,coord_y from Spot where (show_name like '%"+spotname+"%' or keyword like '%"+spotname+"%') and (parent_unit_id='"+unitId+"')");
	...
	public JSONArray around(Double latitude, Double longitude, Double radius, String type, String city){
		...
		try{
			...
			placeList = getHibernateTemplate().find(sql, params);
			jsonObject = new JSONObject();
			Object[] placeObj = null;
			for (int i = 0; i < placeList.size(); i++) {
				placeObj = placeList.get(i);
	...
	public String getMaxModifyTime(){
		...
		try{
			...
			timeList = getHibernateTemplate().find(sql);
			if(!timeList.isEmpty()){
	...
	public JSONArray allSpot(String unit_id,String floor_id){
		//lock.writeLock().lock();
		List<Object[]> placeList = new ArrayList<Object[]>();
		jsonArray = new JSONArray();
		jsonObject = new JSONObject();
		...
		try{
			placeList = hibernateTemplate.find("select show_name,coord_x,coord_y,unit_id from Spot s where s.unit_type_id=1315 and s.parent_unit_id='" + unit_id + "' and s.floor_id = '" + floor_id + "'");
			if (placeList != null && placeList.size() != 0) {
				for(int i = 0; i < placeList.size(); i ++){
					Object[] tempSpot = placeList.get(i);
					jsonObject.put("name", tempSpot[0]);
					jsonObject.put("coord_x", tempSpot[1]);
					jsonObject.put("coord_y", tempSpot[2]);
					jsonObject.put("unit_id", tempSpot[3]);
					jsonArray.add(jsonObject);
				}
			}
			return jsonArray;
